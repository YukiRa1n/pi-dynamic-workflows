...
export const WORKFLOW_NAV_SHORTCUT = "alt+down";
...
function persistedToSnapshot(p) {
    ...
    const snapshotAgents = agents.map((a, callIndex) => {
        ...
        return {
            ...
            status: a.status,
            result,
            resultPreview: result === undefined ? a.resultPreview : String(typeof result === "string" ? result : safeStringify(result)),
            error: a.error,
...
function compactHistoryLine(entry, width) {
    ...
    const value = (key) => (typeof args?.[key] === "string" ? asText(args[key]) : undefined);
    let detail;
...
function renderHistoryEntryLines(history, index, width, markdownTheme, dim, renderCache, summaryOnly = false) {
    const entry = history[index];
...
function resultSummary(result, previewText) {
    if (result === undefined || result === null)
        return oneLine(previewText ?? "") || "no result";
    if (typeof result === "string")
        return oneLine(result) || "no result";
    return oneLine(serializeBounded(result, { pretty: false, maxBytes: 2_000 })) || "structured result";
}
function toolTargetSummary(entry) {
...
function agentActivityBlocks(agent) {
    const blocks = [];
    ...
    for (let index = 0; index < history.length; index++) {
        ...
        if (entry.role === "user" && prompt && text === prompt)
            continue;
        if (entry.kind === "toolCall") {
        ...
        if (text)
            blocks.push({ kind: "context", title: "Input", summary: text, historyIndexes: [index] });
    }
...
function activityCountLabel(blocks) {
...
function fitCompactBody(lines, cap, theme) {
    if (lines.length <= cap)
        return lines;
    if (cap <= 1)
        return [theme.fg("dim", "…")];
...
function renderAgentSummary(agent, width, cap, theme, markdownTheme, renderCache) {
    const blocks = agentActivityBlocks(agent);
    const body = [];
    const label = (text) => theme.fg("dim", `${text.padEnd(9)} `);
    if (agent.error)
        body.push(`${label("Error")}${theme.fg("error", oneLine(agent.error))}`);
    if (agent.errorCode) {
        body.push(`${label("Code")}${theme.fg("error", `${sanitizeUiText(agent.errorCode)}${agent.recoverable ? " · recoverable" : ""}`)}`);
    }
    ...
    if (task)
        body.push(`${label("Task")}${truncateToWidth(task, Math.max(1, width - 11), ELLIPSIS, false)}`);
...
function renderActivityBlockRow(block, selected, width, theme) {
    ...
    if (selected)
        return theme.fg("accent", theme.bold(clipped));
    if (block.kind === "error")
        return theme.fg("error", clipped);
    if (block.kind === "result")
        return theme.fg("success", clipped);
    return clipped;
}
function renderActivityBlockDetails(block, agent, width, markdownTheme, dim, renderCache) {
...
function renderAgentActivity(state, agent, width, cap, theme, markdownTheme, renderCache) {
    const dim = (text) => theme.fg("dim", text);
    const blocks = agentActivityBlocks(agent);
    if (blocks.length === 0)
        return [dim("Activity · waiting for the first agent event")];
    if (state.tailing)
        state.cursor = blocks.length - 1;
    ...
    const range = blocks.length > listCap
    ...
    for (let index = win.start; index < end; index++) {
        ...
        if (block)
            out.push(renderActivityBlockRow(block, index === state.cursor, width, theme));
    }
    if (!state.detailExpanded)
        return out.slice(0, cap);
    const selected = blocks[state.cursor];
    if (!selected || out.length + detailDividerRows >= cap)
        return out.slice(0, cap);
    ...
    if (room <= 0)
        return out.slice(0, cap);
    if (room === 1)
        return [...out, dim("  … detail clipped; full data remains in the run journal")];
...
function footerHint(state, model, theme) {
    ...
    switch (state.kind) {
        case "detail":
            if (state.pagerOpen) {
                parts.push("↑/↓ block", "PgUp/PgDn blocks", "g/G ends", `t tail:${state.tailing ? "on" : "off"}`, `enter ${state.detailExpanded ? "collapse" : "expand"}`, "esc summary");
            }
            else {
        ...
        case "runs": {
            ...
            if (itemKind === "run") {
                ...
                if (status === "running")
                    parts.push("p pause", "x stop");
                else if (status === "paused")
                    parts.push("u resume", "x stop");
                else {
                ...
                switch (action.type) {
                    ...
                    case "resume": {
                        ...
                        void Promise.resolve()
                            ...
                            .then((resumed) => {
                            if (!closed)
                                ui.notify(resumed ? `Resumed ${id}` : `Cannot resume ${id}; inspect its current status.`, resumed ? "info" : "warning");
                        })
                            .catch((error) => {
                            if (!closed)
                                ui.notify(`Resume failed: ${redactForModel(error instanceof Error ? error.message : String(error))}`, "error");
                        })
                            .finally(() => {
                            resuming.delete(id);
                            if (!closed)
                    ...
                    case "restart": {
                        ...
                        if (run.status === "running" || run.status === "paused") {
                            ui.notify(run.status === "paused"
                                ? "Press u to resume this run from its saved progress."
                                : "This run is still active. Pause or stop it before restarting.", "warning");
                            break;
                        }
                        try {
