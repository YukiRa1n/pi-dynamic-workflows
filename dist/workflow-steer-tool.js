...
const workflowSteerSchema = Type.Object({
    ...
    kind: Type.Union([Type.Literal("same_task_correction"), Type.Literal("blocker_answer"), Type.Literal("changed_fact")], {
...
export function createWorkflowSteerTool(options) {
    const getManager = () => {
        const manager = options.getManager?.() ?? options.manager;
        if (!manager)
            throw new Error("workflow_steer: no WorkflowManager configured");
        return manager;
    };
     ...
     */
    const ownedRun = (runId, sessionId, manager) => {
        if (sessionId === undefined)
            return true;
        if (typeof manager.listRuns !== "function")
            return true;
    ...
    return defineTool({
        ...
        async execute(_toolCallId, params) {
            ...
            if (!queuedRunId)
                throw new Error(`Workflow ${params.runId} is not running`);
            return {
...
function normalizeWorkflowSteerInput(value) {
    ...
    if (extraKey)
        throw new Error(`workflow_steer does not accept ${extraKey}`);
