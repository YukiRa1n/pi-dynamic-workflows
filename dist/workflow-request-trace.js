...
export function serializeRequestTrace(payload) {
    const text = JSON.stringify(payload, (key, value) => {
        if (/^(authorization|api[_-]?key|access[_-]?token|refresh[_-]?token|headers|encrypted_content|signature|thinkingSignature)$/i.test(key))
            return "[omitted]";
        if (typeof value === "string") {
            if (/^data:[^;]+;base64,/.test(value) || (key === "data" && value.length > 1024))
...
export function requestTracePath(sessionId) {
...
export function appendRequestTrace(path, record) {
    const dir = join(path, "..");
    mkdirSync(dir, { recursive: true });
    const line = `${JSON.stringify(record)}\n`;
    if (Buffer.byteLength(line) > MAX_LOG_BYTES)
        throw new Error("trace record exceeds log limit");
...
export function installWorkflowRequestTrace(pi) {
    ...
    const write = (stage, data, ctx) => {
        ...
        try {
            ...
            if (!failed)
                ctx?.ui?.notify?.(`Workflow request trace failed: ${String(error)}`, "warning");
            failed = true;
        }
    };
    ...
    pi.on("context", (event) => {
        ...
        projection = event.messages
            ...
            .slice(-16)
            .map((message) => ({ tool: message.toolName, id: message.toolCallId, content: message.content }));
    });
    ...
    pi.on("provider_request_prepared", (event, ctx) => {
        preparedSeen = true;
        if (!enabled)
            return;
        write("provider-request-prepared", { ...serializeRequestTrace(event.payload), model: ctx.model?.id, provider: ctx.model?.provider }, ctx);
    });
    ...
    pi.on("message_end", (event, ctx) => {
        ...
        if (event.message.role === "custom" && event.message.customType.startsWith("workflow-")) {
            write("history-delivery", serializeRequestTrace({
        ...
        write("assistant-response", {
            ...
            ...serializeRequestTrace(event.message.content.filter((part) => part.type === "text" || part.type === "toolCall")),
        }, ctx);
    });
    pi.registerCommand("workflow-trace", {
        ...
        handler: async (args, ctx) => {
            if (args.trim() === "off")
                ...
                enabled = true;
            ctx.ui.notify(`Request trace ${enabled ? "on" : "off"}; final payload ${preparedSeen ? "observed" : "not observed yet"}; ${requestTracePath(sessionId || ctx.sessionManager.getSessionId())}`, "info");
        },
    });
}
