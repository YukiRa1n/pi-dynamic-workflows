...
import { createGetWorkflowOutputTool, createListActiveWorkflowsTool, createStopWorkflowTool, createWorkflowControlTool, } from "./workflow-control-tool.js";
...
function validateWorkflowControlSurface() {
    ...
    if (parameters.type !== "object" || parameters.additionalProperties !== false) {
        diagnostics.push(diagnostic(WorkflowReleaseDiagnosticCode.TOOL_INPUT_MISMATCH, "workflow_control", "workflow_control must expose a strict top-level object schema for provider compatibility."));
    }
    const action = properties.action;
    const actionNames = isRecord(action) && Array.isArray(action.anyOf)
        ? action.anyOf.flatMap((entry) => {
...
function validateStopWorkflowSurface() {
    ...
    const properties = isRecord(parameters.properties) ? parameters.properties : {};
    const diagnostics = [];
    if (tool.name !== "stop_workflow" ||
        parameters.type !== "object" ||
        parameters.additionalProperties !== false ||
        Object.keys(properties).length !== 1 ||
        !isRecord(properties.runId)) {
        diagnostics.push(diagnostic(WorkflowReleaseDiagnosticCode.TOOL_INPUT_MISMATCH, "stop_workflow", "stop_workflow must expose only one exact runId in a strict top-level object schema."));
    }
    if (/\b(?:list|status|pause|resume|steer|latest)\b/iu.test(tool.description)) {
        diagnostics.push(diagnostic(WorkflowReleaseDiagnosticCode.TOOL_INPUT_MISMATCH, "stop_workflow.description", "stop_workflow description must not advertise discovery, inferred targeting, or broader lifecycle actions."));
    }
    return diagnostics;
}
function validateListActiveWorkflowsSurface() {
    ...
    const properties = isRecord(parameters.properties) ? parameters.properties : {};
    const diagnostics = [];
    if (tool.name !== "list_active_workflows" ||
        parameters.type !== "object" ||
        parameters.additionalProperties !== false ||
        Object.keys(properties).length !== 0) {
        diagnostics.push(diagnostic(WorkflowReleaseDiagnosticCode.TOOL_INPUT_MISMATCH, "list_active_workflows", "list_active_workflows must expose a strict empty object schema."));
    }
    if (/\b(?:history|progress|details?|pause|resume|steer)\b/iu.test(tool.description)) {
        diagnostics.push(diagnostic(WorkflowReleaseDiagnosticCode.TOOL_INPUT_MISMATCH, "list_active_workflows.description", "list_active_workflows description must stay limited to current-session cancellation handles."));
    }
    return diagnostics;
}
function validateGetWorkflowOutputSurface() {
    ...
    const diagnostics = [];
    if (tool.name !== "get_workflow_output" ||
        parameters.type !== "object" ||
        parameters.additionalProperties !== false ||
        Object.keys(properties).sort().join(",") !== "block,runId" ||
        required.length !== 1 ||
        required[0] !== "runId") {
        diagnostics.push(diagnostic(WorkflowReleaseDiagnosticCode.TOOL_INPUT_MISMATCH, "get_workflow_output", "get_workflow_output must expose exact runId plus optional block in a strict object schema."));
    }
    if (!/event wait/iu.test(tool.description) ||
        !/not status/iu.test(tool.description) ||
        !/never poll/iu.test(tool.description) ||
        !/shell sleep/iu.test(tool.description)) {
        diagnostics.push(diagnostic(WorkflowReleaseDiagnosticCode.TOOL_INPUT_MISMATCH, "get_workflow_output.description", "get_workflow_output must describe a next-output event wait, not a status check or list/shell polling."));
    }
    return diagnostics;
}
function validateModelWorkflowToolIsolation() {
    ...
    if (missing.length === 0)
        return [];
    return [
        diagnostic(WorkflowReleaseDiagnosticCode.TOOL_INPUT_MISMATCH, "DEFAULT_EXCLUDED_SUBAGENT_TOOLS", `Workflow subagents must not receive model-facing orchestration tools: ${missing.join(", ")}.`),
    ];
}
...
function validateChangedSurfaces(root, publishableFiles, changedFiles) {
    ...
    for (const changed of candidates.map(normalizePackagePath)) {
        if (!RELEASE_SURFACE_SOURCES.includes(changed))
            continue;
        const source = join(root, changed);
        if (!existsSync(source)) {
            diagnostics.push(diagnostic(WorkflowReleaseDiagnosticCode.STALE_GENERATED_SURFACE, changed, `Changed release surface is missing from the source tree: ${changed}.`));
            continue;
        }
        if (!published.has(changed)) {
            diagnostics.push(diagnostic(WorkflowReleaseDiagnosticCode.MISSING_PACKAGE_RESOURCE, changed, `Changed release surface is not included in the publishable package: ${changed}.`));
        }
        ...
        for (const generated of [`dist/${basename}.js`, `dist/${basename}.d.ts`]) {
            ...
            if (stale) {
                diagnostics.push(diagnostic(WorkflowReleaseDiagnosticCode.STALE_GENERATED_SURFACE, generated, `Changed source surface ${changed} has no current publishable generated twin ${generated}; run the TypeScript build before release.`));
            }
        }
    }
    return diagnostics;
