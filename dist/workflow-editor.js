...
export function registerWorkflowProgressCommands(pi, settingsStore = DEFAULT_SETTINGS_STORE) {
    pi.registerCommand?.("workflows-progress", {
        ...
        async handler(args, _ctx) {
            ...
            if (verb === "icons") {
                if (!rest) {
                    await say(`Progress panel glyphs are ${loadProgressIcons(settingsStore)}. Usage: /workflows-progress icons auto | ascii`);
                    return;
                }
            ...
            await say(`Workflow progress panel is ${loadProgressMode(settingsStore)}, showing up to ${loadProgressMaxAgents(settingsStore)} agents per phase. Usage: /workflows-progress compact | detailed | status | max <N> | icons auto | ascii`);
        },
    });
    pi.registerCommand?.("workflows-demo", {
        ...
        async handler(args, ctx) {
            ...
            if (mode && mode !== "live") {
                ctx.ui.notify("/workflows-demo [live | preview]：默认在主会话运行真实演示，使用两个子代理和模型 token。", "info");
                return;
            }
            if (!ctx.isIdle()) {
...
function loadProgressIcons(settingsStore) {
    try {
