...
export function resolveModelForPhase(phase, config) {
    for (const route of config.routes) {
        if (route.useRegex)
            validatePhasePattern(route.phasePattern);
    }
...
function validatePhasePattern(pattern) {
    if (typeof pattern !== "string") {
        throw new WorkflowError(`Invalid phasePattern regex ${String(pattern)}: pattern must be a string`, WorkflowErrorCode.SCRIPT_VALIDATION_ERROR, { recoverable: false });
    }
    if (pattern.length > MAX_PHASE_PATTERN_LENGTH) {
        throw new WorkflowError(`Invalid phasePattern regex "${pattern}": pattern exceeds the ${MAX_PHASE_PATTERN_LENGTH}-character limit`, WorkflowErrorCode.SCRIPT_VALIDATION_ERROR, { recoverable: false });
    }
    ...
    if (NESTED_QUANTIFIER_PATTERN.test(pattern)) {
        throw new WorkflowError(`Unsafe phasePattern regex "${pattern}": nested quantifiers are not allowed`, WorkflowErrorCode.SCRIPT_VALIDATION_ERROR, { recoverable: false });
    }
    if (QUANTIFIED_ALTERNATION_PATTERN.test(pattern)) {
        throw new WorkflowError(`Unsafe phasePattern regex "${pattern}": quantified alternation groups are not allowed`, WorkflowErrorCode.SCRIPT_VALIDATION_ERROR, { recoverable: false });
    }
    try {
        new RegExp(pattern, "i");
    }
    catch (error) {
        throw new WorkflowError(`Invalid phasePattern regex "${pattern}": ${error instanceof Error ? error.message : String(error)}`, WorkflowErrorCode.SCRIPT_VALIDATION_ERROR, { recoverable: false });
    }
}
/**
