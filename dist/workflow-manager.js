...
import { DEFAULT_MAX_PAUSED_BYTES_ON_DISK, DEFAULT_MAX_PAUSED_RUNS_IN_MEMORY, DEFAULT_MAX_PAUSED_RUNS_ON_DISK, DEFAULT_WORKFLOW_TIMEOUT_MS, MAX_AGENT_PROMPT_BYTES, MAX_PENDING_DELIVERIES_PER_RUN, MAX_PENDING_MESSAGE_BYTES, MAX_PENDING_MESSAGES, } from "./config.js";
...
import { WorkflowResourceCoordinator, } from "./workflow-resource-coordinator.js";
const WORKFLOW_STEERING_KINDS = new Set([
...
export class WorkflowManager extends EventEmitter {
    ...
    resourceNamespace;
    maxPausedRunsOnDisk;
    ownedWorktreeTokens = new Set();
    maxPausedBytesOnDisk;
    ...
    constructor(options = {}) {
        ...
        this.maxPausedBytesOnDisk = Math.max(1, Math.floor(options.maxPausedBytesOnDisk ?? DEFAULT_MAX_PAUSED_BYTES_ON_DISK));
        ...
        this.resources =
            options.resourceCoordinator ??
                new WorkflowResourceCoordinator({
                    ...
                    maxQueuedProviderAttempts: options.maxQueuedProviderAttempts ?? options.resourceLimits?.maxQueuedProviderAttempts,
    ...
    markPersistenceBlocked(managed) {
        managed.persistenceBlocked = true;
        managed.error = new WorkflowError("Workflow state could not be durably published", WorkflowErrorCode.PERSISTENCE_ERROR, { recoverable: true });
        ...
        if (managed.executionSettled === true)
    ...
    enqueueUserMessage(message, runId, kind) {
        ...
        if (!WORKFLOW_STEERING_KINDS.has(kind))
            return undefined;
        ...
        if (queue.length >= 256 || queuedBytes + textBytes > 1_000_000)
            return undefined;
    ...
    startInBackground(script, args, exec = {}) {
        ...
        const resourceExecutionReservation = this.resources.acquireExecution(runId, this.resourceNamespace, executionGeneration);
        if (!resourceExecutionReservation) {
            throw new WorkflowError("Maximum active workflow execution capacity has been reached", WorkflowErrorCode.RESOURCE_LIMIT_EXCEEDED, { recoverable: false });
        }
    ...
    async runSync(script, args, exec = {}) {
        ...
        const executionGeneration = generateRunId();
        const resourceExecutionReservation = this.resources.acquireExecution(managed.runId, this.resourceNamespace, executionGeneration);
        if (!resourceExecutionReservation) {
            throw new WorkflowError("Maximum active workflow execution capacity has been reached", WorkflowErrorCode.RESOURCE_LIMIT_EXCEEDED, { recoverable: false });
        }
    ...
    admitExplicitDelivery(managed, message, alertKind) {
        ...
        if (managed.deliveryOutbox.length >= MAX_PENDING_DELIVERIES_PER_RUN - 2 ||
     ...
     */
    admitAgentDelivery(runId, input) {
        assertSafeRunId(runId);
        const managed = this.runs.get(runId);
        if (!managed || !this.isCurrent(managed) || !managed.background)
            return undefined;
    ...
    ensurePausedDelivery(managed, error) {
        if (!managed.background || managed.status !== "paused")
            return undefined;
        if (managed.deliveryOutbox.length >= MAX_PENDING_DELIVERIES_PER_RUN - 1)
            return undefined;
        ...
        const delivery = {
            ...
            status: "pending",
            checkpoint: "paused",
            content: `⏸ Background workflow ${managed.runId} paused: ${error.message}${when}. ` +
    ...
    markDeliverySubmitted(deliveryId, generation) {
        if (!this.bridgeDeliveryState)
            this.bridgeDeliveryState = new Map();
        ...
        while (this.bridgeDeliveryState.size > 1024) {
            const oldest = this.bridgeDeliveryState.keys().next().value;
            if (oldest === undefined)
                break;
    ...
    discardDelivery(runId, deliveryId) {
        assertSafeRunId(runId);
        const managed = this.runs.get(runId);
        if (managed) {
        ...
        if (!this.ownsPersistedRun(state) || !state)
            return false;
    ...
    async executeRun(managed, script, args, exec = {}) {
        ...
        const resolvedWorkflowTimeoutMs = managed.workflowTimeoutMs !== undefined
            ? managed.workflowTimeoutMs
        ...
        try {
            ...
            const result = await runWorkflow(script, {
                ...
                onAgentSessionEnd: ({ id, session }) => {
                    ...
                    if (target?.managed === managed && target.session === session)
                        this.activeAgentSenders.delete(id);
                },
                ...
                initialTokenUsage,
                providerAcquire: (runId, signal) => this.resources.acquireProvider(runId, signal, this.resourceNamespace, managed.executionGeneration ?? runId),
                lateAttemptRegistry: {
            ...
            if (result.status === "exhausted") {
                ...
                managed.error = new WorkflowError("All agent calls failed after retries; the workflow result is best-effort at best. Review the agents tab and edit/resume the script to retry.", WorkflowErrorCode.AGENT_EXECUTION_ERROR, { recoverable: true });
            ...
            const workflowError = error instanceof WorkflowError
                ? error
                : new WorkflowError(error instanceof Error ? error.message : String(error), intentionalAbort ? WorkflowErrorCode.WORKFLOW_ABORTED : WorkflowErrorCode.UNKNOWN, { recoverable: intentionalAbort });
            ...
            if (terminalPersistenceError) {
                throw new WorkflowError(terminalPersistenceError instanceof Error
                    ? terminalPersistenceError.message
                    : String(terminalPersistenceError), WorkflowErrorCode.PERSISTENCE_ERROR, { recoverable: true });
            }
            throw workflowError;
        }
    }
     ...
     */
    releaseExecutionCapacity(managed) {
        if (managed.resourceExecutionReleased)
            return;
     ...
     */
    cleanupSettledGeneration(managed) {
        if (!this.isCurrent(managed))
            return;
        const retentionStatus = IN_MEMORY_TERMINAL_STATUSES.has(managed.status)
            ...
            : managed.status === "paused"
                ? "paused"
                : undefined;
        if (!retentionStatus || managed.settledCleanupStatus === retentionStatus)
            return;
    ...
    persistedStateFor(managed) {
        ...
        return {
            ...
            failure: (managed.status === "failed" || managed.status === "paused") && managed.error
                ? {
    ...
    async resume(runId, opts) {
        ...
        const managed = {
            ...
            workflowTimeoutMs: persisted.workflowTimeoutMs !== undefined ? persisted.workflowTimeoutMs : this.defaultWorkflowTimeoutMs,
    ...
    prunePausedRuns(options) {
        const protectedRunIds = new Set([...this.runs.values()]
    ...
    deleteRun(runId) {
        ...
        try {
            ...
            if (fallbackLease)
                this.persistence.releaseRunLease(fallbackLease);
        }
    }
    /**
    ...
    syncPersistedRevision(runId, revision) {
        if (!Number.isSafeInteger(revision) || revision === undefined)
            return;
        const managed = this.runs.get(runId);
        if (managed && (managed.revision ?? 0) < revision)
