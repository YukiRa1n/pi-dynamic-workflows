...
import { claimWorkflowAgentOutput, isWorkflowAgentOutputConsumed, workflowAgentOutputCandidates, } from "./workflow-agent-output-source.js";
...
const WORKFLOW_OUTPUT_AGENT_EVENT = "agentEnd";
const WORKFLOW_OUTPUT_PARENT_INPUT_EVENT = "parentInput";
...
function redactWorkflowText(value) {
        ...
        async execute(_toolCallId, params, signal, onUpdate) {
            ...
            try {
                ...
                if (outcome === "agent") {
                    ...
                    if (!afterAgent)
                        return workflowOutputError(params.runId, block, "run was deleted while waiting");
                    if (afterAgent.status !== "running")
                        return workflowOutputState(manager, afterAgent, block, options);
                ...
                if (outcome === "input") {
                    return workflowOutputState(manager, current, block, options, {
                        inputPending: true,
                        message: "User input arrived while waiting. End this wait and process the queued user message at Pi's normal post-tool steering boundary; the workflow continues in the background.",
                    });
                }
                return workflowOutputState(manager, current, block, options, {
    ...
    return defineTool({
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                ...
                if (!run) {
                    return controlError(params.action, params.runId, sessionId ? "run not found in current session" : "run not found", []);
                }
...
function waitForWorkflowOutput(manager, runId, sessionId, signal) {
    return new Promise((resolve, reject) => {
        ...
        const onParentInputEvent = (event) => {
            if (isRecord(event) && event.runId === runId)
                finish("input");
        };
        ...
        if (!settled) {
            ...
            if (current?.status !== "running")
                finish("ready");
            else if (hasUnconsumedAgentOutputs(manager, current))
                finish("agent");
            else {
                try {
...
export function releaseWorkflowOutputWaitForInput(manager, runId) {
    manager.emit(WORKFLOW_OUTPUT_PARENT_INPUT_EVENT, { runId });
}
function workflowAgentOutputState(manager, run, blocked, options, outputs, hasMore) {
...
function hasUnconsumedAgentOutputs(manager, run) {
    return agentOutputCandidates(manager, run).some((output) => !isWorkflowAgentOutputConsumed(manager, run.runId, output, output.value));
}
function claimAgentOutputs(manager, run) {
    const pending = agentOutputCandidates(manager, run).filter((output) => !isWorkflowAgentOutputConsumed(manager, run.runId, output, output.value));
...
function agentOutputCandidates(manager, run) {
...
function liveRunFailure(manager, runId) {
    try {
...
function projectWorkflowFailure(error) {
...
function summarizeRun(run, live, managed) {
    ...
    return {
        ...
        counts,
        activeLabels: run.status === "running"
