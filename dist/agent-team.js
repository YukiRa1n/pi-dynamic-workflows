...
export class WorkflowAgentTeam {
    ...
    async waitForInbox(memberId, options = {}) {
        ...
        const waiters = this.inboxWaiters.get(memberId) ?? new Set();
        const cleanup = () => {
            if (waiter.timer)
            ...
            if (waiters.size === 0)
                this.inboxWaiters.delete(memberId);
        };
        waiter.onAbort = () => {
            cleanup();
    ...
    createTools(memberId, attemptGen, isAdmitted) {
        ...
        return [
            ...
            defineTool({
                ...
                parameters: Type.Object({
                    waitMs: Type.Optional(Type.Integer({ minimum: 1, maximum: 600_000, description: "Wait for a peer message for this many ms." })),
                }, { additionalProperties: false }),
                async execute(_toolCallId, params, signal) {
                    assertAdmitted();
                    const waited = params.waitMs === undefined
