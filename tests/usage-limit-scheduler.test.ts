  ...
  asRunPersistence(): RunPersistence {
    return {
      save: (state: PersistedRunState) => {
        const revision = (this.runs.get(state.runId)?.revision ?? 0) + 1;
        state.revision = revision;
        this.runs.set(state.runId, { ...state, revision, updatedAt: new Date().toISOString() });
      },
      load: (runId: string) => this.runs.get(runId) ?? null,
      list: () => [...this.runs.values()],
      delete: (runId: string) => this.runs.delete(runId),
      acquireRunLease: (runId: string) => ({ runId, token: `lease-${runId}` }),
      releaseRunLease: () => {},
...
class FakeManager extends EventEmitter implements SchedulableWorkflowManager {
  ...
  syncPersistedRevision(runId: string, revision: number | undefined): void {
    const run = this.persistence.get(runId);
...
test("attempts and opt-out are persisted (best-effort) for a future cold start", async () => {
  ...
  manager.emit("paused", { runId: "run-1", reason: "usage_limit", resetHint: "resets in 10m" });
  // Mirror WorkflowManager: it publishes the paused event, then synchronously
  // persists paused state before the scheduler's queued microtask executes.
  const afterEvent = manager.persistence.get("run-1");
  if (afterEvent) afterEvent.status = "paused";
  await flush();
