...
test("createTeam and deliver retain frozen authoring coverage until scenario evidence exists", () => {
  const entriesById = new Map(WORKFLOW_AUTHORING_COVERAGE.map((entry) => [entry.id, entry]));

  for (const name of ["createTeam", "deliver"] as const) {
    const entry = entriesById.get(`workflow.runtime.${name}`);
    assert.equal(entry?.protection, WorkflowAuthoringProtection.GUIDANCE_FROZEN);
    assert.deepEqual(entry?.comprehensionScenarios, []);
    assert.ok((entry?.protectedGuidance.length ?? 0) > 0);
  }
});
