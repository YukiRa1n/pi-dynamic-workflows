...
describe("registerSavedWorkflow", () => {
  ...
  it("falls back to runWorkflow (inline) when no manager is provided", async () => {
    ...
    const wf = {
      name: "run-inline",
      script:
        "export const meta = { name: 't', description: 't' };\nconst report = await agent('return done');\nreturn { report };",
    };
    const fakeHome = mkdtempSync(join(tmpdir(), "pi-dw-home-"));
    // Keep this test deterministic and offline: the saved-command path should
    // pass through runWorkflow, but it must not require a configured provider.
    const { WorkflowAgent } = await import("../src/agent.js");
    const run = mock.method(WorkflowAgent.prototype as any, "run", async () => "done");
    try {
