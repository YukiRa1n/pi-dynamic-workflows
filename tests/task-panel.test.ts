...
describe("installResultDelivery", () => {
  ...
  it("falls back to bounded JSON when result is an object with no known key", () => {
    const pi = createMockPi();
    ...
    assert.ok(calls[0].content.includes("middle omitted"), "should mark bounded head/tail projection");
  });
  ...
  it("falls back gracefully when result is nullish", () => {
    ...
    const calls = (pi as unknown as { _calls: { content: string }[] })._calls;
    assert.equal(calls.length, 1);
    assert.ok(calls[0].content.includes("[undefined]"), "should represent an undefined semantic return explicitly");
  });
  ...
  it("refreshes the live delivery settings loader across reload generations", () => {
    ...
    assert.match(content, /middle omitted/, "the reused listener reads settings from the fresh generation");
  });
});
...
describe("deliverText", () => {
  ...
  it("defaults the provider projection threshold to 12,000 chars", async () => {
    const { deliverText } = await import("../src/task-panel.js");
    const under = deliverText(makeResult({ note: "y".repeat(11_900) }) as never);
