...
test("one generated supported-capability table is fresh across skill, README, and website docs", () => {
  ...
  assert.doesNotMatch(generatedTable, /\| background \|/);
  assert.doesNotMatch(generatedTable, /background\?: boolean/);
  assert.match(
...
test("generated helper facts expose exact callback, option, result, and failure contracts", () => {
  ...
  const lifecycle = readFileSync(join(ROOT, SKILL_ROOT, "references/lifecycle.md"), "utf8");
  assert.match(lifecycle, /public workflow tool invocations are always backgrounded/i);
  assert.match(lifecycle, /headless/i);
  assert.doesNotMatch(lifecycle, /backgrounded by default/i);
});
