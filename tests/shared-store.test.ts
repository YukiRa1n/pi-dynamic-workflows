...
test("SharedStore.trackPut + commitDelta tracks per-agent writes", () => {
  ...
  // After commit the one-shot attempt windows are retired, not reopenable.
  assert.throws(() => store.commitDelta("run-1:2"), /already completed/);
  assert.throws(() => store.commitDelta("run-1:3"), /already completed/);
});
...
test("SharedStore.dispose clears state and fences all later callbacks", () => {
  const store = new SharedStore();
  store.put("k", "v");
  store.trackPut("k2", "v2", "run-1:1");
  store.dispose();
  assert.throws(() => store.get("k"), /disposed/);
  assert.throws(() => store.commitDelta("run-1:1"), /disposed/);
});
...
test("SharedStore.discardDelta rolls back to the pre-window value, not an intermediate write", () => {
  ...
  assert.throws(
    () => store.commitDelta("run-1:0"),
    /already completed/,
    "a discarded attempt is retired and cannot be reopened or committed",
  );
});
...
test("resume after nested workflow(): parent and child journal entries cache-hit independently despite both indexing at 0", async () => {
  ...
  // workflow() itself reserves a parent lexical call index, so the child's own
  // first agent remains index 0 while the subsequent parent agent is index 1.
  // Distinct runIds remain the primary frame namespace.
  assert.deepEqual(journal.map((e) => e.index).sort(), [0, 1]);
...
test("resume degrades gracefully (never corrupts) when replaying a pre-namespacing legacy journal", async () => {
  ...
  // The current lexical workflow() slot means these entries use indices 0 and
  // 1, so even after stripping runId they no longer collapse. Older truly
  // colliding journals remain safely handled by the manager's last-entry map
  // semantics, but this newly-produced fixture exercises the non-colliding path.
  assert.equal(legacyResumeJournal.size, 2, "distinct lexical indices survive runId stripping");
