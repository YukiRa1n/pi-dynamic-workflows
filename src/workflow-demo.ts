...
export function buildLiveWorkflowDemoPrompt(): string {
...
const DEMO_RUN_IDS = ["workflow-demo-compact", "workflow-demo-paused", "workflow-demo-detailed"] as const;
...
interface DemoManagerShape {
  listRuns(): PersistedRunState[];
  getRun(runId: string): DemoLiveRun | undefined;
}
...
function agent(
  ...
  return {
    ...
    tokens,
    tokenUsage:
      tokens > 0 ? usage(Math.floor(tokens * 0.3), Math.floor(tokens * 0.2), Math.floor(tokens * 0.5)) : undefined,
    model: "anthropic/claude-sonnet-4-5",
    error: status === "error" ? "Example validation failure" : undefined,
  };
}

function snapshot(name: string, agents: WorkflowAgentSnapshot[], currentPhase = "Checks"): WorkflowSnapshot {
  return {
    name,
    phases: [currentPhase],
    currentPhase,
...
function asWorkflowManager(runs: PersistedRunState[], liveRuns: DemoLiveRun[]): WorkflowManager {
  const liveById = new Map(liveRuns.map((run) => [run.snapshot.runId ?? "", run]));
  const manager: DemoManagerShape = {
    listRuns: () => runs,
    getRun: (runId) => liveById.get(runId),
  };
  return manager as unknown as WorkflowManager;
}

function compactDemoManager(elapsedMs: number): WorkflowManager {
  const movingTokens = 2_000 + Math.floor(elapsedMs * 1.8);
  const runningAgents = Array.from({ length: 5 }, (_, index) =>
    agent(index + 1, `check-${index + 1}`, "running", movingTokens + index * 320),
  );
...
function detailedDemoManager(elapsedMs: number): WorkflowManager {
  const agents = [
...
export function renderWorkflowDemo(
  theme: Theme,
  width: number,
  now = Date.now(),
  startedAt = now,
...
export function openWorkflowDemo(ui: ExtensionUIContext, iconMode: IconMode = "auto"): Promise<void> {
  for (const runId of DEMO_RUN_IDS) clearTokenSamples(runId);

  return ui.custom<void>(
    (tui: TUI, theme: Theme, _keybindings, done: (result: undefined) => void) => {
      ...
      const component: Component & { handleInput(data: string): void; dispose(): void } = {
        render: (width: number) => {
          const innerWidth = Math.max(1, width - 2);
          const title = " Workflow UI demo ";
          const titleText = truncateToWidth(title, innerWidth, "", true);
          const top = `╭${theme.fg("accent", titleText)}${theme.fg(
            "borderMuted",
            "─".repeat(Math.max(0, innerWidth - visibleWidth(titleText))),
          )}╮`;
          ...
          const background = (line: string) => theme.bg("customMessageBg", line);
          return [background(top), ...body.map(background), background(bottom)];
        },
        handleInput: (data: string) => {
          if (data === "q" || data === "Q" || data === "\x1b" || data === "\x03") close();
        },
        invalidate: () => {},
        dispose: cleanup,
      };
