  ...
  for (const forbidden of ["list", "status"]) {
    if (actionNames.includes(forbidden)) {
      diagnostics.push(
        diagnostic(
          WorkflowReleaseDiagnosticCode.TOOL_INPUT_MISMATCH,
          `workflow_control.action.${forbidden}`,
          `workflow_control must not expose polling action ${forbidden}; inspection belongs to the user-facing /workflows command and completions arrive through workflow-result steering.`,
        ),
      );
    }
  }
  ...
  if (!isRecord(runId) || runId.type !== "string" || runId.minLength !== 1 || !requiredProperties.includes("runId")) {
    diagnostics.push(
      diagnostic(
        ...
        "workflow_control runId must be a required non-empty string for pause, resume, and stop.",
      ),
    );
  }
  return diagnostics;
