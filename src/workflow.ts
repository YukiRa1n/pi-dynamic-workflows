  ...
  const settleStoreDelta = (order: number, delta: Record<string, unknown>): void => {
    ...
    while (shared.pendingStoreDeltas.has(shared.nextStoreOrder)) {
      ...
      try {
        ...
        appendLog(
          `shared-store delta ${shared.nextStoreOrder} rejected at commit: ${
      ...
      try {
        ...
        if (code === WorkflowErrorCode.TOKEN_BUDGET_EXHAUSTED || code === WorkflowErrorCode.AGENT_LIMIT_EXCEEDED) {
          ...
          shared.agentFailures.push({
            ...
            errorCode: (code as WorkflowErrorCode | undefined) ?? WorkflowErrorCode.RESOURCE_LIMIT_EXCEEDED,
            recoverable: false,
          });
          break;
        }
...
for (const name of Object.getOwnPropertyNames(globalThis)) {
  ...
  try {
    ...
    const status =
      failures.length === 0 ? "completed" : (shared.successfulAgentCount ?? 0) === 0 ? "exhausted" : "partial";
