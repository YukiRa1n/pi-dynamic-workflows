...
import { WorkflowAgentTeam, type AgentTeamSnapshot, type AgentTeamSpawnSpec } from "./agent-team.js";
...
function createCacheWarmGate(): CacheWarmGate {
...
function createParentMessageTool(
  ...
  return defineTool({
    ...
    description:
      "Send a genuinely important finding, blocker, or decision to the main session immediately. Use sparingly; routine progress belongs in your final result.",
    parameters: Type.Object(
      { message: Type.String({ minLength: 1, description: "Important message for the main session." }) },
    ...
    fn
      ? (...args: A) => {
          try {
            ...
            logger.warn(
              `workflow observer "${name}" threw: ${err instanceof Error ? err.message : String(err)}`,
            );
          }
        }
      : undefined;
            ...
            const runPromise = agentRunner.run(agentPrompt, {
              ...
              systemTools: [
                ...
                createParentMessageTool(
                  ...
                  label,
                  options.onDeliver,
                  () => myGen === attemptSeq && !agentController.signal.aborted,
                ),
                ...(team ? team.createTools(agentOptions.teamMember as string, attemptGen) : []),
              ],
              cwd: runCwd,
              ...
              onUsage: (u: AgentUsage) => {
                ...
                if (shared.admission === "closed" || accounting.usage) return;
                accounting.usage = u;
...
function hashAgentCall(
  ...
  const identity = JSON.stringify({
    ...
    retries:
      options.retries === undefined ? "inherited" : options.retries === null ? "disabled" : options.retries,
