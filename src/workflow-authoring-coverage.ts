...
export const WORKFLOW_AUTHORING_FROZEN_FILES = [
  {
    ...
    sha256: "651c3501a4eac06816cc91daa843616d544171ee950723f290ce86fae40945e4",
  },
  {
...
const FROZEN_GUIDANCE_BY_CAPABILITY: Readonly<Record<string, readonly ProtectedGuidanceSurface[]>> = {
  ...
  "workflow.runtime.checkpoint": [
    ...
    },
    {
      path: LIFECYCLE_PATH,
      requiredText:
        "Public workflow tool invocations are always backgrounded and therefore headless: they cannot display checkpoint confirmation.",
    },
  ],
  ...
  "workflow.runtime.args": [
    {
      path: RUNTIME_PATH,
      requiredText:
        "The runtime supplies `agent`, `parallel`, `pipeline`, `createTeam`, `workflow`, quality/control helpers, `phase`, `log`, `args`, `cwd`, restricted `process.cwd()`, and `budget`.",
    },
  ],
