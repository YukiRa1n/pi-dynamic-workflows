...
export const WORKFLOW_ROUTING_CHOICE_SCENARIOS: readonly WorkflowRoutingChoiceScenario[] = [
  {
    id: "new-request-stays-direct",
    prompt:
      "Workflow run-old-a is still auditing authentication. The user's new, unrelated request is: explain in one sentence what a mutex is.",
    expectedTool: null,
  },
  {
    id: "explicit-new-workflow-starts-fresh",
    prompt:
      "Workflow run-old-a is still auditing authentication. The user now explicitly asks: run a new workflow to review the storage layer from three independent angles.",
    expectedTool: "start_workflow",
  },
  {
    id: "same-task-update-stays-out-of-model-tools",
    prompt:
      "The user provides a correction for the same authentication audit: workflow run-old-a should inspect config/auth-v2.json instead of config/auth.json. Apply this correction to that run.",
    expectedTool: null,
  },
  {
    id: "ambiguous-runs-never-guessed",
    prompt:
      "Two unrelated workflows, run-old-a and run-old-b, are active. The user's new request is: rename a local variable in the current file. No run is identified as the target.",
    expectedTool: null,
  },
];

export function evaluateWorkflowRoutingChoice(
  scenario: WorkflowRoutingChoiceScenario,
  calls: readonly CapturedWorkflowRoutingCall[],
  session?: WorkflowRoutingSessionEvidence,
  ...
  const scriptEvidence =
    scenario.expectedTool === "start_workflow" ? parseCapturedStart(args) : { passed: true, details: "not applicable" };
  const unsupportedStartFields = calls.flatMap(({ name, arguments: value }) => {
    if (name !== "start_workflow" || !isRecord(value)) return [];
    return [
      "maxAgents",
      "concurrency",
      "agentRetries",
  ...
  const assertions = [
    ...
    },
    {
      ...
      details:
        unsupportedStartFields.length === 0
...
function parseCapturedStart(args: Record<string, unknown> | undefined): { passed: boolean; details: string } {
...
function parseCapturedScript(value: unknown): { passed: boolean; details: string } {
 ...
 * parser above.
 */
function normalizeScriptForEvidence(script: string): string {
