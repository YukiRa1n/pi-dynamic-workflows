...
async function assertSafeFetchUrl(raw: string): Promise<URL> {
  ...
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") throw new Error("web_fetch only permits http(s) URLs");
  ...
  if (addresses.length === 0 || addresses.some(blockedAddress)) throw new Error("web_fetch rejects private or local network targets");
  return parsed;
}

async function fetchText(url: string, timeoutMs = 15000, maxBytes = 2_000_000): Promise<{ status: number; body: string }> {
  ...
  try {
    ...
    for (let hop = 0; hop <= 5; hop++) {
      ...
      if (res.status >= 300 && res.status < 400) {
        ...
        if (!location) throw new Error(`redirect ${res.status} has no location`);
      ...
      try {
        while (true) {
          ...
          if (bytes > maxBytes) throw new Error(`web response exceeds ${maxBytes} bytes`);
