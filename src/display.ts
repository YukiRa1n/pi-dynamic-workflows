...
export function renderWorkflowLines(
  ...
  for (const agent of snapshot.agents) {
    ...
    else byPhase.set(agent.phase, [agent]);
  }
  ...
  for (const phase of phaseNames) {
    ...
    for (const agent of agents) {
      rendered.add(agent);
      if (agent.status === "done") done++;
      else if (agent.status === "running") running++;
      else if (agent.status === "error") errors++;
    ...
    lines.push(
      theme.fg("accent", `  ${marker} ${terminalText(phase)}`) +
        theme.fg(
          "dim",
...
function statusLine(snapshot: WorkflowSnapshot, completed: boolean): string {
  ...
  if (completed) return `workflow ✓ ${name}: ${snapshot.doneCount}/${snapshot.agentCount}`;
