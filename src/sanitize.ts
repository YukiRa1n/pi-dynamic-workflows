...
const C0_AND_C1_PATTERN = new RegExp(
  `[${String.fromCharCode(0)}-${String.fromCharCode(8)}${String.fromCharCode(11)}-${String.fromCharCode(31)}${String.fromCharCode(127)}${String.fromCharCode(128)}-${String.fromCharCode(159)}]`,
  "g",
);

const COMMON_TOKEN_PATTERN =
  /\b(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_-]+\b|\bgithub_pat_[A-Za-z0-9_-]+\b|\bsk-[A-Za-z0-9][A-Za-z0-9_-]*\b/g;
...
const SECRET_ASSIGNMENT_PATTERN =
  /((?:authorization|api[-_]?key|access[-_]?token|password)["']?\s*[:=]\s*)(?:"(?:Bearer\s+)?(?:\\.|[^"\\])*"|'(?:Bearer\s+)?(?:\\.|[^'\\])*'|(?:Bearer\s+)?[^\s;'"`]+)/gi;
...
function truncateUtf8(text: string, maxBytes: number): string {
  ...
  if (maxBytes <= 0) return "";
  if (markerBytes >= maxBytes) return prefixUtf8(TRUNCATION_MARKER, maxBytes);
...
function homeDirectoryPrefixes(): string[] {
  try {
    const environment = typeof process === "undefined" ? undefined : process.env;
    const windowsHome =
      environment?.HOMEDRIVE && environment.HOMEPATH ? `${environment.HOMEDRIVE}${environment.HOMEPATH}` : "";
    return [...new Set([environment?.HOME, environment?.USERPROFILE, windowsHome])].filter(
...
function homePrefixPattern(prefix: string): string {
...
function redactHomeDirectories(text: string): string {
  let output = text;
 ...
 */
export function redactAbsolutePaths(text: string, placeholder = "[path redacted]"): string {
  if (typeof text !== "string") return "";
  try {
    return text.replace(ABSOLUTE_PATH_PATTERN, (match, prefix: string | undefined) =>
      match.startsWith("file://") ? `${placeholder}` : `${prefix ?? ""}${placeholder}`,
    );
  } catch {
    return "";
  }
...
export function sanitizeForTerminal(text: string): string {
  ...
  try {
    let output = text;
    for (const pattern of CONTROL_SEQUENCE_PATTERNS) output = output.replace(pattern, "");
 ...
 * throwing.
 */
export function redactForModel(text: string, maxBytes?: number): string {
  if (typeof text !== "string") return "";
  try {
...
export function sanitizeAndRedact(text: string, maxBytes?: number): string {
