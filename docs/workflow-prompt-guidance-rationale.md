...
## Current placement in 3.5.1-yuki.4
...
Background execution is no longer a caller-selectable parameter. Keeping one public path prevents foreground/inline behavior from diverging in schema, result delivery, checkpoint semantics, or release documentation. The tool result and the lifecycle section of the authoring skill are the authoritative model-facing explanations; the generated capability contract intentionally contains no `background` input.
...
Confidence is very high that the always-background tool path, its result-delivery behavior, and the absence of a public timing toggle are the stable contract.
